...
## Assistara visual language
- Use Manrope throughout. The deck should feel like the Assistara website and Academy interface translated into a teaching artifact—not a branded PowerPoint template.
- Core palette: near-black `#151515`, warm cream `#FBFAF6`, Assistara yellow `#FFD51F`. Supporting teaching states may use soft sage, peach, powder blue and lavender.
- Use purposeful soft gradient fields when they create the same depth and energy as the website hero: blue/lavender/yellow or peach/yellow light behind a dark product surface. Do not use generic gradients as page decoration.
- Alternate cream, near-black and full-yellow foundations to create rhythm. Use yellow for the decisive idea, current state, active control or key teaching signal.
- Use oversized editorial typography, short line lengths, generous whitespace and high contrast.
- Use dark product/system panels, floating supporting cards, soft shadows and small controlled rotations when showing digital work. These are signature Assistara devices, not effects to scatter everywhere.
- Yellow underline bands may emphasize the key phrase in a cover or section headline.
...
- Avoid corporate PowerPoint styling, generic AI deck aesthetics, random gradients, stock-photo filler, decorative icon grids and walls of text.
...
- Default placement is the **top right**, matching the established Academy decks. The mark should render at roughly 40–48 px on a 1280×720 slide, with the wordmark large enough to read in the embedded lesson viewer.
- Keep the lockup in one consistent position, scale and safe margin throughout a deck. It must be unmistakably visible at phone size while remaining secondary to the teaching headline.
...
- For software lessons, use clean, realistic simplified interfaces with believable labels and states. Build them as high-contrast product surfaces with clear information hierarchy. Do not paste screenshots with irrelevant chrome or draw interfaces that misrepresent how the tool works.
